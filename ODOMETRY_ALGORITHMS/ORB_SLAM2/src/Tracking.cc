...
// ros::NodeHandle n;
// ros::Publisher chatter_pub = n.advertise<std_msgs::Int32>("abc_topic", 1000);
...
int Tracking::GetTheReliabilityOfTheCamera()
{
    return Camera_Reliability;
}
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp,int id)
{
    mImGray = imRectLeft;
 ...
 */
bool Tracking::TrackWithMotionModel(int id)
{
    ...
    if(nmatches<20){
        Camera_Reliability = nmatches;
        //cout << "TrackWithMotionModel 1 "<< id <<" = " << Camera_Reliability <<"\n";
        return false;
    }
    ...
    if(mbOnlyTracking)
    {
        mbVO = nmatchesMap<10;
        Camera_Reliability = nmatches;
        //cout << "TrackWithMotionModel 2 "<< id <<" = " << Camera_Reliability <<"\n";
        return nmatches>20;
    }
    Camera_Reliability = nmatchesMap;
    //cout << "TrackWithMotionModel 3 "<< id <<" = " << Camera_Reliability <<"\n";
    return nmatchesMap>=10;
}
/**
        ...
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                if(nGood<50)
                {
                    ...
                    cout << "TrackWithMotionModel nadditional = " << nadditional <<"\n";
                    Camera_Reliability = nadditional;
                }
