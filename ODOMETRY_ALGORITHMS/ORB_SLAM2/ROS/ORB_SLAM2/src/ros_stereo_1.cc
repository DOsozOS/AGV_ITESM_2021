...
int main(int argc, char **argv)
{
    ...
    ORB_SLAM2::System SLAM(argv[1],argv[2],ORB_SLAM2::System::STEREO,id,true, bool(std::string(argv[5]) == "true"));
    /*******Load Map*********/
    std::string OnlyTracking = std::string(argv[5]);
    if (OnlyTracking == "true")
    {
        SLAM.LoadMap("map1.bin");
    ...
    message_filters::Subscriber<sensor_msgs::Image> left_sub(nh, "/camera1/left/image_raw", 1);
    message_filters::Subscriber<sensor_msgs::Image> right_sub(nh, "/camera1/right/image_raw", 1);
    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image> sync_pol;
    message_filters::Synchronizer<sync_pol> sync(sync_pol(10), left_sub,right_sub);
    sync.registerCallback(boost::bind(&ImageGrabber::GrabStereo,&igb,_1,_2));

    ros::Publisher PosPub = nh.advertise<geometry_msgs::PoseStamped>("ORB_SLAM2_1/pose", 5); // Pose publisher
        igb.pPosPub = &(PosPub); //
    ros::Publisher OdomPub = nh.advertise<nav_msgs::Odometry>("ORB_SLAM2_1/odom", 5); //Odometry publisher
        igb.pOdomPub = &(OdomPub); //
    ros::Publisher QualityPub = nh.advertise<std_msgs::Int32>("ORB_SLAM2_1/quality", 5); // Pose publisher
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    if(do_rectify)
    {
        ...
        cv::Mat Tcw= mpSLAM->TrackStereo(imLeft,imRight,cv_ptrLeft->header.stamp.toSec(),1);  //
        camera1_reliability = mpSLAM->GetTheReliabilityOfTheCamera_system();
        //cout<<"Ros Reliability = "<<camera1_reliability<<"\n";
        PublishPose(Tcw); //
        PublishOdom(Tcw); //
        (pQualityPub)->publish(camera1_reliability);
    }
    else
    {
        cv::Mat Tcw= mpSLAM->TrackStereo(cv_ptrLeft->image,cv_ptrRight->image,cv_ptrLeft->header.stamp.toSec(),1); //
        camera1_reliability = mpSLAM->GetTheReliabilityOfTheCamera_system();
        //cout<<"Ros Reliability = "<<camera1_reliability<<"\n";
        PublishPose(Tcw); //
        PublishOdom(Tcw);
