	...
	while not (rospy.is_shutdown()):
		...
		if(GLOBAL['READY_DATA'] == 1 and GLOBAL['WORKING_CMD'] == 6 and firstime == False):
			if(prevtime != 0):
				...
				theta_range = math.radians(30) #35 DEGREES WILL BE CONSIDERED AS MINOR CORRECTION
				if(abs(desired_theta)<theta_range):#(positive_angles(math.atan2(tfc_y,tfc_x)) <theta_range or positive_angles(math.atan2(tfc_y,tfc_x))>(2*PI-theta_range)):
					GLOBAL['KH']=0.3
					print("LOW CORRECTION")
				else:
					GLOBAL['KH']=0.7
				...
				if(finished==False):
					...
					GLOBAL['controller_ST'] = steering_bound(GLOBAL['KH'] *desired_theta)*57.296
				...
				print("desired theta:"+str(desired_theta))
